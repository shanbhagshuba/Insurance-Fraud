...
class Predict_From_Model:
    ...
    def prediction(self):
        ...
        try:
            ...
            final_data,num_cols = self.data_preprocessing_obj.feature_engineering(self.file_object,data, unwanted_columns,self.Image_Folder_Prediction,'Prediction')
            ...
            X = self.data_preprocessing_obj.feature_scaling_splitting(self.file_object,final_data,None,'Mixed',mixed_minmax_cols , num_cols)
            ...
            for i in range(0,Tot_Clusters):
                ...
                clf = self.load_model(cluster_folder_name,best_estimator)  
                result  =(clf.predict(cluster_features))
                for res in result:
                    if res==0:
...
if __name__ == "__main__":
    ...
    prediction_validation_insertion_obj.prediction_validation_insertion('Insurance','tbl_prediction')
    prediction_obj.prediction()
    
    # def get_correct_model(folder_name):
    #     try:
    #         files = glob.glob(Model_Folder + folder_name+ "\\" +"*.sav", recursive=True)
    #         best_estimator = files[0].split('\\')[-1].split('.')[0]
    #         return best_estimator
    #     except Exception as e:
    #         raise Exception()
    ...
    #             cluster_features = cluster_features.drop(['Cluster'],axis=1)
    #             cluster_folder_name = 'Cluster ' + str(i)
    #             best_estimator = get_correct_model(cluster_folder_name)
    #             clf = load_model(cluster_folder_name,best_estimator)  
    #             result  =(clf.predict(cluster_features))
    #             for res in result:
    #                 if res==0:
    #                     predictions.append('N')
